...
/**
 * Checks whether the given fix is valid or not.
 *
 * @param {any} fix The value to check.
...
module.exports = (function() {
    ...
    api.report = function(ruleId, severity, node, location, message, opts, fix, meta) {
        ...
        if (isValidFix(fix) || (Array.isArray(fix) && fix.length >= 1 && fix.every(isValidFix))) {
            assert(!meta || meta.fixable, "Fixable rules should export a `meta.fixable` property.");

            problem.fixes = Array.isArray(fix) ? fix : [fix];

            // make `fix` property for backward compatibility.
            if (problem.fixes.length === 1) {
                problem.fix = problem.fixes[0];
            }
        }
