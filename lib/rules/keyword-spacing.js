...
module.exports = {
    ...
    create(context) {
        ...
        function containsOnlyOneSpace(first, second) {
            const text = sourceCode.getText().slice(first.range[1], second.range[0]);

            return /^[ ]{1}$/.test(text.replace(/\/\*.*?\*\//g, ""));
        }
        ...
        function expectSpaceBefore(token, pattern) {
            ...
            if (prevToken &&
                ...
                !containsOnlyOneSpace(prevToken, token)
            ) {
                ...
                context.report({
                    ...
                    data: token,
                    fix(fixer) {
                        // Replace space between prevToken end location and token start location with spaces.
