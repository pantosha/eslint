...
SourceCodeFixer.applyFixes = function(sourceCode, messages) {
    ...
    // Extract fixes from problems.
    for (const problem of messages) {
        if (!problem.hasOwnProperty("fixes")) {
            remainingMessages.add(problem);
            continue;
        }
    ...
    if (fixes.length) {
        ...
        for (const fix of fixes.sort(compareFixesByRange)) {
            const problem = fix.problem;
            const start = fix.start;
            const end = fix.end;

            // Remain it as a problem if it's overlapped.
            if (lastPos >= start || remainingMessages.has(problem)) {
                remainingMessages.add(problem);
                continue;
            }
        ...
        return {
            fixed: true,
            messages: Array
                .from(remainingMessages)
                .sort(compareMessagesByLocation),
            output
        };
    }
